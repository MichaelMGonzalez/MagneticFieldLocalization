...
public class SerialConsoleActivity extends Activity implements AdapterView.OnItemSelectedListener{
    ...
    private HashMap<String, Byte> msgMap;
    ...
    private void initMsgContainers() {
        // Setup the hashmap
        msgMap = new HashMap<>(SerialMsgHeader.MSG_KEYS.length);
        for( int i = 0; i < SerialMsgHeader.MSG_KEYS.length; i++)
            msgMap.put(SerialMsgHeader.MSG_KEYS[i], SerialMsgHeader.MSG_VALS[i]);
        // Setup the spinner
        msgSpinner = (Spinner) findViewById(R.id.msgSpinner);
        ArrayAdapter<String> adapter = new ArrayAdapter<String>(this,
                android.R.layout.simple_spinner_dropdown_item, SerialMsgHeader.MSG_KEYS);
        msgSpinner.setAdapter(adapter);
        msgSpinner.setOnItemSelectedListener(this);
    }
    ...
    public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
        byte[] msgBuffer = { SerialMsgHeader.MSG_VALS[position] };
        try {
            sPort.write(msgBuffer, 1000);
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
